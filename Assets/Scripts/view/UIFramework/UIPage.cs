...
namespace tpgm.UI
{
    ...
    public abstract class UIPage 
    {
		...
		{
			//#提示: 服务器数据出错, 请重新登录;
			//Toast.create(" 服务器数据出错, 请重新登录");
		}

		//#服务器出现逻辑错误;
		public void serverLogicError(int code)
		{
			//Toast.create("respCode: " + code);
		}

		//#游戏数据异常: 该有的数据没有;
		public void gameDataCorrupt(Exception ex)
		{
		...
		static void queueToast(Params p)
		{
			if (m_queue.Count > 0)
			{
			}
		...
		static void showToast(string text, float sec)
		{
		...
		public static void create(string text, float sec)
		{
			Params p = new Params(text);
			p.m_sec = sec;
