...
namespace Pomelo.DotNetClient
{
    ...
    public class Protocol
    {
		...
		private const string CRYPT_KEY = "0123456789abcd0123456789";
		private const bool ENCRTYP_ENABLE = false;
        ...
        internal void send(PackageType type, byte[] body)
        {
			...
			if (null != body && body.Length > 0) {
				if (ENCRTYP_ENABLE) {
					body = CryptUtils.Encrypt3DesBytes (CRYPT_KEY, body);
				}
        ...
        internal void processMessage(byte[] bytes)
        {
			...
			if (null != pkg.body && pkg.body.Length > 0) {
				if (ENCRTYP_ENABLE) {
					pkg.body = CryptUtils.Decrypt3DesBytes(CRYPT_KEY, pkg.body);
				}
